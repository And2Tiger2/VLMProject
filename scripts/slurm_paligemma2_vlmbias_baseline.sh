#!/usr/bin/env bash
#SBATCH --job-name=pg2-vlmbias-base
...
OUT_DIR="${OUT_DIR:-runs/paligemma2_vlmbias_baseline}"
MODEL_ID="${MODEL_ID:-google/paligemma2-3b-mix-448}"
LIMIT="${LIMIT:-}"
SEEDS="${SEEDS:-0 1 2 3 4}"
CONDITIONS="${CONDITIONS:-image blank_image}"
...
cmd=(
  uv run --extra paligemma python scripts/run_paligemma2_vlmbias_baseline.py
  --dataset "$DATASET"
  --out-dir "$OUT_DIR"
  --model-id "$MODEL_ID"
