...
from adapters.qwen25_vl_attention import make_adapter
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep image-token attention alpha on VLMBias and NaturalBench.")
    ...
    parser.add_argument("--out-dir", default="runs/attention_alpha_sweep")
...
def _run_vlmbias_condition(adapter, examples, out_dir: Path, condition: dict[str, Any], seed: int, run_config: dict):
    ...
    out_path = out_dir / "vlmbias" / f"qwen25vl_3b_attention_{condition_name}_seed{seed}.jsonl"
...
def _run_naturalbench_condition(adapter, calls, out_dir: Path, condition: dict[str, Any], seed: int, run_config: dict):
    condition_name = condition["condition"]
    out_path = out_dir / "naturalbench" / f"qwen25vl_3b_attention_{condition_name}_seed{seed}.jsonl"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    predictions = []
    row_dicts = []
    with out_path.open("w", encoding="utf-8") as handle:
