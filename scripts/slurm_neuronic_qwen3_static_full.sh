#!/usr/bin/env bash
#SBATCH --job-name=q3-gaze-static
...
OUT_DIR="$SEGMENT_ROOT/runs/static_narration_seed${SEED}_top${TOP_K}_${START_COMIC_IDX}_${SHARD_SIZE}"
...
echo "host=$(hostname) gpu=${CUDA_VISIBLE_DEVICES:-unset} seed=$SEED top_k=$TOP_K start=$START_COMIC_IDX"
...
srun uv run python scripts/run_qwen25_gaze_static_narration.py \
  ...
  --top-k-gaze "$TOP_K" \
  --top-k-random "$TOP_K" \
  --control-mode "${CONTROL_MODE:-paper}" \
  --targets-per-strip 6 \
  --max-new-tokens 100 \
  --no-decode-only \
  --seed "$SEED" \
