...
def main()->None:
    ...
    ablation_path=Path(config["head_ablation"]);require_current_artifact(ablation_path);inputs.append(ablation_path);double_table=args.output_dir/"causal_double_dissociation.tsv";double_figure=args.output_dir/"causal_double_dissociation.png";render_double_dissociation(read_tsv(ablation_path),double_table,double_figure)
    ...
    outputs=[table,figure_path,report,score_figure,double_table,double_figure]+([centroid_figure] if centroid_figure else []);write_run_manifest(args.output_dir,config=config,seeds={"render":args.seed},inputs=inputs,outputs=outputs,status="complete",repo_root=Path.cwd());print(json.dumps({"valid":True,"rows":len(rows),"outputs":[str(path) for path in outputs]},indent=2))
