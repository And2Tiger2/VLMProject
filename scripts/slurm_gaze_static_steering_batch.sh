...
TOP_K="${TOP_K:?Set TOP_K, e.g. 1, 5, 10, or 20}"
SEGMENT_ROOT="${SEGMENT_ROOT:-segments/gaze_heads_qwen25}"
COMICS_ROOT="${COMICS_ROOT:-$SEGMENT_ROOT/data/comics}"
GAZE_RANKING="${GAZE_RANKING:-$SEGMENT_ROOT/runs/gaze_discovery_merged_0_500/gaze_head_ranking.json}"
MODEL_ID="${MODEL_ID:-Qwen/Qwen2.5-VL-3B-Instruct}"
DEVICE_MAP="${DEVICE_MAP:-cuda}"
TARGETS_PER_STRIP="${TARGETS_PER_STRIP:-6}"
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-100}"
OUT_DIR="${OUT_DIR:-$SEGMENT_ROOT/runs/static_narration_top${TOP_K}_${START_COMIC_IDX}_${MAX_COMICS}}"
...
cmd=(
  ...
  --top-k-gaze "$TOP_K"
  --top-k-random "$TOP_K"
  --targets-per-strip "$TARGETS_PER_STRIP"
