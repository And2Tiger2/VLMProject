#!/usr/bin/env bash
#SBATCH --job-name=attn-alpha-sweep
...
OUT_DIR="${OUT_DIR:-runs/attention_alpha_sweep}"
VLMBIAS_LIMIT="${VLMBIAS_LIMIT:-}"
NATURALBENCH_LIMIT_GROUPS="${NATURALBENCH_LIMIT_GROUPS:-}"
SEEDS="${SEEDS:-0}"
ALPHAS="${ALPHAS:-0.05 0.1 0.25 0.5 1.0 2.0 3.0}"
...
cmd=(
  uv run python scripts/run_attention_alpha_sweep.py
