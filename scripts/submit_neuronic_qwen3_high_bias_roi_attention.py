...
STAGE_COUNTS = {"smoke": 6, "tune": 12, "heads": 12, "confirm": 6}
STAGE_ORDER = ("smoke", "tune", "heads", "confirm")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit the staged Qwen3 Game-Board/Optical-Illusion ROI experiment."
    )
    ...
    if not args.dry_run:
        (repo / "segments/vlm_bias_attention/runs/slurm").mkdir(parents=True, exist_ok=True)
    ...
    result: dict[str, Any] = {
        "experiment": "qwen3_high_bias_roi_attention_v1",
        "stage": args.stage,
        "jobs": jobs,
...
def _submit_stage(
    ...
    *,
    ...
    prepare = submitter.sbatch(
        "scripts/slurm_neuronic_qwen3_high_bias_roi_prepare.sh",
        exports=stage_exports,
        dependency=dependency,
    )
    run = submitter.sbatch(
        "scripts/slurm_neuronic_qwen3_high_bias_roi_condition.sh",
        exports=stage_exports,
        dependency=prepare,
        array=f"0-{count - 1}%{min(count, max_parallel)}",
    )
    aggregate = submitter.sbatch(
        "scripts/slurm_neuronic_qwen3_high_bias_roi_aggregate.sh",
        exports=stage_exports,
        dependency=run,
    )
...
def _preflight(repo: Path, stage: str) -> None:
    required = [
        ...
        repo / "scripts/slurm_neuronic_qwen3_high_bias_roi_prepare.sh",
        repo / "scripts/slurm_neuronic_qwen3_high_bias_roi_condition.sh",
        repo / "scripts/slurm_neuronic_qwen3_high_bias_roi_aggregate.sh",
    ]
    ...
    if len(vlmbias) != 114 or len({str(row["id"]) for row in vlmbias}) != 114:
        raise SystemExit("Expected exactly 114 unique high-bias-topic VLMBias rows")
    if {str(row["topic"]) for row in vlmbias} != {"Game Boards", "Optical Illusion"}:
        raise SystemExit("Runtime VLMBias slice has unexpected topics")
    source_by_id = {str(row["id"]): row for row in vlmbias}
    accepted = read_jsonl(repo / DEFAULT_ROI_ROOT / "accepted.jsonl")
    if len(accepted) != 114 or len({str(row["group_id"]) for row in accepted}) != 79:
        raise SystemExit("Expected 114 reviewed masks in 79 canonical visual groups")
    if {str(row["topic"]) for row in accepted} != {"Game Boards", "Optical Illusion"}:
        raise SystemExit("High-bias mask manifest has unexpected topics")
    for row in accepted:
        source = source_by_id.get(str(row["id"]))
        if source is None:
            raise SystemExit(f"Mask ID absent from VLMBias slice: {row['id']}")
        image = Path(str(source["image_path"]))
        if not image.is_absolute():
            image = repo / DEFAULT_VLMBIAS.parent / image
        if not image.is_file():
            raise SystemExit(f"Missing VLMBias image: {image}")
        for variant in ("tight", "broad"):
            mask = repo / DEFAULT_ROI_ROOT / row["artifacts"][f"{variant}_mask"]
            if not mask.is_file():
                raise SystemExit(f"Missing {variant} ROI mask: {mask}")
