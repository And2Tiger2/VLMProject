...
def main()->None:
    ...
    source=Path(config["per_example_scores"]);require_current_artifact(source);rows=[row for row in read_tsv(source) if row.get("excluded","").casefold() not in {"true","1"}];groups=sorted({row["group_id"] for row in rows})
    if len(groups)<2:raise RuntimeError("MACI stability requires at least two independent groups")
    halves=(set(groups[::2]),set(groups[1::2]));scores=[aggregate([row for row in rows if row["group_id"] in half]) for half in halves];comparison=compare(scores[0],scores[1],name="split_half",k=int(config.get("top_k",40)));comparisons=[comparison];inputs=[args.config,source]
    for index,path_value in enumerate(config.get("repeat_per_example_scores",[])):
        path=Path(path_value)
        if path.is_file():require_current_artifact(path);inputs.append(path);comparisons.append(compare(aggregate(rows),aggregate([row for row in read_tsv(path) if row.get("excluded","").casefold() not in {"true","1"}]),name=f"cross_seed_{index}",k=int(config.get("top_k",40))))
    ...
    summary={"valid":True,"label":"instrumentation smoke test" if args.smoke else ("methods-based reproduction" if passes else "failed calibration"),"passes_stability_gate":passes,"thresholds":{"minimum_spearman":minimum_rho,"minimum_sign_agreement":minimum_sign},"comparisons":comparisons,"claim_gate":"Driving/resisting labels remain provisional unless this gate and locked ablations pass."};summary_path=args.output_dir/"summary.json";summary_path.write_text(json.dumps(summary,indent=2),encoding="utf-8");write_run_manifest(args.output_dir,config={**config,"smoke":args.smoke},seeds={"analysis":args.seed},inputs=inputs,outputs=[output,summary_path],status="complete",repo_root=Path.cwd());print(json.dumps(summary,indent=2))
