...
mkdir -p "$CACHE_ROOT/uv" "$CACHE_ROOT/huggingface" "$CACHE_ROOT/torch" \
  "$REPO/segments/vlm_bias_attention/runs/slurm"
cd "$REPO"
...
echo "host=$(hostname) gpu=${CUDA_VISIBLE_DEVICES:-unset} task=${SLURM_ARRAY_TASK_ID:-0}"
uv run python scripts/check_neuronic_gpu.py \
  --min-memory-gb "${MIN_GPU_MEMORY_GB:-20}"
srun uv run python scripts/run_qwen3_roi_context_factorial_condition.py \
