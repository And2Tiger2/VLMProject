#!/usr/bin/env bash
#SBATCH --job-name=vlmbias-2pass-attn
...
OUT_DIR="${OUT_DIR:-runs/vlmbias_two_pass_prompt_attention}"
LIMIT="${LIMIT:-}"
...
cmd=(
  uv run python scripts/run_vlmbias_two_pass_prompt_attention.py
  --dataset "$DATASET"
  --out-dir "$OUT_DIR"
)
