...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare one Qwen3 VLMBias ROI-attention stage."
    )
    ...
    parser.add_argument("--dev-groups", type=int, default=50)
    parser.add_argument("--smoke-groups", type=int, default=8)
    args = parser.parse_args()

    splits = prepare_roi_splits(
    ...
    if args.stage == "smoke":
        ...
        conditions, split = (
            head_conditions(float(tune["selected"]["spec"]["alpha"])),
            "dev",
        )
    else:
        tune = _read_json(args.report_root / "tune" / "selection.json")
